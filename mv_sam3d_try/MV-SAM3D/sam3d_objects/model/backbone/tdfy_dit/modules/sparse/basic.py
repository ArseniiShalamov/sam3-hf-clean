...
class SparseTensor:
    """
    Sparse tensor with support for torchsparse, spconv, and dense fallback backend.

    In dense fallback mode, we store only feats/coords and skip backend-specific sparse ops.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if method_id == 0:
            ...
            if BACKEND == "torchsparse":
                ...
                self._dense_feats = feats
                self._dense_coords = coords
            ...
            if BACKEND == "dense":
                raise ValueError("Dense backend does not support constructing SparseTensor from backend data object")
    ...
    def dense(self) -> torch.Tensor:
        if BACKEND == "torchsparse":
            ...
            raise NotImplementedError("Dense fallback SparseTensor.dense() is not implemented")
