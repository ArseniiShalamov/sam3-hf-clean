...
try:
    ...
    def check_tensor(x, shape, throw=False):
        """
        Lightweight fallback for non-Kaolin environments.
        Checks tensor rank and fixed dimensions where shape entries are not None.
        """
        import torch
        ...
        if len(x.shape) != len(shape):
            if throw:
                raise ValueError(f"Expected rank {len(shape)}, got {len(x.shape)}")
            return False

        for got, expected in zip(x.shape, shape):
            if expected is not None and got != expected:
                if throw:
                    raise ValueError(f"Expected shape like {shape}, got {tuple(x.shape)}")
                return False
